...
export default function TrafficImportDetailPage() {
  ...
  useEffect(() => {
    setLoading(true);
    fetch(`/api/onsi/traffic/${importId}`)
      .then((r) => r.json())
      .then((d) => {
  ...
  if (!imp) {
    return (
      <main className="mx-auto max-w-5xl px-4 py-6 sm:px-6">
        ...
        <Link href="/onsi/traffic-data" className="text-sm text-navy hover:underline">
  ...
  return (
    <main className="mx-auto max-w-5xl px-4 py-6 sm:px-6">
      <Link href="/onsi/traffic-data" className="text-xs font-medium text-ink-soft hover:text-navy">
        ← Traffic Data
      </Link>
