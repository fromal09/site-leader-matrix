...
import { onsiDcSiteHref as dcSiteHref, ONSI_DC_BASE as DC_BASE } from "@/lib/onsiRoutes";
...
function DepthChartsHomeInner() {
  ...
  const availableDivisions = Array.from(new Set(allSites.map((s) => s.division)))
    .sort()
  ...
  async function loadSummary(periodKey?: string) {
    const url = periodKey
      ? `/api/onsi/depth-chart-writers/all-sites-summary?period=${encodeURIComponent(periodKey)}`
      : "/api/onsi/depth-chart-writers/all-sites-summary";
  ...
  useEffect(() => {
    Promise.all([fetch("/api/onsi/sites").then((r) => r.json()), loadSummary()])
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="grid grid-cols-1 gap-6 lg:grid-cols-[1fr_300px]">
            ...
            <div>
              <WriterLeaderboard division={division} period={selectedPeriod} apiPrefix="/onsi" />
            </div>
          </div>
        </>
      )}
