...
export async function GET(
  ...
  try {
    ...
    const rows = await sql`
      SELECT at.article_title, at.first_published_date, at.pageviews::float8 AS pageviews,
    ...
    const topArticles = [...latestRows].sort((a, b) => b.pageviews - a.pageviews).slice(0, 5);
    ...
    return NextResponse.json({
      matched: true,
      matchName,
      latestPeriodLabel: latestRows[0]?.period_label,
      topArticles,
      recentArticles,
      monthly,
    });
