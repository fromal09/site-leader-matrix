...
export async function GET(
  ...
  try {
    ...
    // If this site's raw onsi_article_traffic for this period has been
    ...
    if (rowsArr.length === 0) {
      const archiveRows = await sql`
        SELECT * FROM onsi_site_traffic_archive WHERE site_id = ${siteIdNum} AND period_key = ${selectedPeriodKey}
      `;
    ...
    if (archivedSite) {
      const writerArchiveRows = await sql`
        SELECT * FROM onsi_writer_traffic_archive
