...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    // If a writer's raw onsi_article_traffic for their site's relevant period has
    // been archived (see /api/admin/archive-old-traffic), they won't have
    // any rows in byKey above. onsi_writer_traffic_archive is already keyed by
    ...
    const archiveRows =
      relevantPeriods.length > 0
        ? await sql`
            ...
            FROM onsi_writer_traffic_archive
