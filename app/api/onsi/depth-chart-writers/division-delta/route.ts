...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const sitesInDivision = await sql`
      SELECT id, site_name, leader_name FROM onsi_sites WHERE division = ${division}
    `;
    ...
    const importDates = await sql`
      SELECT site_id, imported_at FROM onsi_traffic_imports
    ...
    const writers = await sql`
      ...
      LEFT JOIN onsi_depth_chart_roles dr ON TRIM(LOWER(dr.label)) = TRIM(LOWER(dcw.role))
    ...
    const prevSiteRows = await sql`
      ...
      FROM onsi_site_traffic_snapshots
      WHERE site_id = ANY(${siteIds}::int[]) AND period_key = ${periodKey}
