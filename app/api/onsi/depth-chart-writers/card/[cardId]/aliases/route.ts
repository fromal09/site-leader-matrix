...
export async function GET(
  ...
  try {
    const rows = await sql`
      SELECT id, alias FROM onsi_writer_aliases WHERE writer_id = ${Number(cardId)} ORDER BY alias ASC
    `;
...
export async function POST(
  ...
  try {
    const rows = await sql`
      INSERT INTO onsi_writer_aliases (writer_id, alias, created_by)
    ...
    const existing = await sql`
      SELECT id, alias FROM onsi_writer_aliases WHERE writer_id = ${Number(cardId)} AND alias = ${alias.trim()}
    `;
