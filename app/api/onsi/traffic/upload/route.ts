...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const existing = await sql`
      SELECT id FROM onsi_traffic_imports WHERE site_id = ${siteIdNum} AND period_key = ${periodKey}
    `;
    let importId: number;
    if ((existing as any[])[0]) {
      ...
      const outgoingRows = await sql`
        ...
        FROM onsi_article_traffic WHERE import_id = ${importId}
      `;
      ...
      if (outgoing.length > 0) {
        ...
        await sql`
          INSERT INTO onsi_site_traffic_snapshots
        ...
        for (const w of writers as any[]) {
          ...
          await sql`
            INSERT INTO onsi_writer_traffic_snapshots
        ...
        await sql`
          INSERT INTO onsi_site_daily_deltas (site_id, period_key, pv_delta, scroll_weighted_sum_delta, time_weighted_sum_delta)
        ...
        for (const w of writers as any[]) {
          ...
          await sql`
            INSERT INTO onsi_writer_daily_deltas
      ...
      await sql`DELETE FROM onsi_article_traffic WHERE import_id = ${importId}`;
      await sql`
        UPDATE onsi_traffic_imports
      ...
      const inserted = await sql`
        INSERT INTO onsi_traffic_imports (site_id, period_key, period_label, row_count, imported_by)
    ...
    for (let i = 0; i < rows.length; i += CHUNK_SIZE) {
      ...
      await sql`
        INSERT INTO onsi_article_traffic
